...
% Input Files
fn_rdf = fullfile(output_dir, 'RDF.nii');
fn_phase = fullfile(output_dir, 'unwrapped.nii');
fn_complex = fullfile(output_dir, 'complex.nii');
...
qsm_nii = make_nii(QSM);
qsm_nii.hdr.dime.pixdim(2:4) = voxel_size;
fn_qsm = fullfile(output_dir, 'qsm_tkd.nii');
save_nii(qsm_nii, fn_qsm);
...
if nd == 1
    ...
    tol = 0.15; % 0.15 checking impact 0.25
else
    tol = 1;
end
...
qsm_nii = make_nii(QSM);
qsm_nii.hdr.dime.pixdim(2:4) = voxel_size;
fn_qsm = fullfile(output_dir, 'qsm_ndi.nii');
save_nii(qsm_nii, fn_qsm);
